...
async function seedDatabase() {
    ...
    await query(`INSERT INTO users (email, password_hash, first_name, last_name, role, company_name, subscription_tier, subscription_status)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, ['demo@buildtrack.com', passwordHash, 'Demo', 'User', 'admin', 'BuildTrack Demo', 'pro', 'active']);
    const userResult = await query('SELECT id FROM users WHERE email = $1', ['demo@buildtrack.com']);
    const userId = userResult.rows[0].id;
    // Insert demo project
    await query(`INSERT INTO projects (user_id, name, description, location, budget, progress, status, start_date, end_date)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [userId, 'Downtown Office Complex', 'Construction of a 12-story office building in downtown', '123 Main St, City', 2500000, 45, 'active', '2024-01-15', '2025-06-30']);
    ...
    for (const [title, desc, priority, status] of tasks) {
        await query(`INSERT INTO tasks (project_id, title, description, priority, status, due_date)
       VALUES ($1, $2, $3, $4, $5, $6)`, [projectId, title, desc, priority, status, new Date(Date.now() + 30 * 86400000)]);
    }
    // Insert demo workers
    const workers = [
    ...
    for (const [name, role, phone, email] of workers) {
        await query(`INSERT INTO workers (user_id, name, role, phone, email, hourly_rate, weekly_hours)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [userId, name, role, phone, email, 25 + Math.floor(Math.random() * 30), 40]);
    }
